...
A V1 já possui shell Electron, interface escura, bandeja do sistema, inicialização opcional com o Windows, até quatro sessões isoladas, layouts em grade/linha/coluna, modo leve que oculta os painéis sem interromper o farm, polling de estado, métricas de farm, Hunt Analyzer com preferência por dados do servidor, tierlist modular estimada, calculadora de IV, userscripts locais explícitos, equipe sanitizada por conta, inventário sanitizado por conta, histórico local de hunts encerradas, exportação CSV, alertas configuráveis de queda/sem progresso/sem Pokébolas com notificação nativa opcional e webhook Discord opcional, login da conta do jogo com Turnstile manual, Market/Depot, compra/venda protegida, recuperação cidade → operação → hunt e licenciamento assinado. Webhooks são aceitos apenas via HTTPS nos domínios oficiais do Discord e ficam criptografados localmente; a URL nunca é devolvida à interface. Os contratos privados do jogo ainda precisam de validação com uma conta real antes de serem tratados como compatibilidade de produção.
