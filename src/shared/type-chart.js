...
function typeEffectiveness(moveType, defenderTypes) {
  const chart = TYPE_CHART[String(moveType || '').toUpperCase()] || {};
  return (Array.isArray(defenderTypes) ? defenderTypes : []).filter(Boolean).reduce((value, type) => value * (chart[String(type).toUpperCase()] ?? 1), 1);
}
