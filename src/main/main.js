...
ipcMain.handle('account:action', async (event, id, action, input) => { if (!isTrustedUi(event)) return { ok: false, reason: 'forbidden' }; const adapter = gameAdapters.get(String(id)); if (!adapter || !['openDepot', 'readDepot', 'openMarket', 'travelToHunt', 'returnToLastHunt', 'buyBalls', 'sellItems', 'sellPokemon', 'sellStone', 'detectGameLogin', 'fillGameCredentials', 'submitGameLogin'].includes(String(action))) return { ok: false, reason: 'action_not_allowed' }; try { return { ok: true, result: await adapter[action](input || {}) }; } catch (cause) { return { ok: false, reason: cause.code || 'action_failed' }; } });
