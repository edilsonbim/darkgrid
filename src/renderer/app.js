...
const statusLabels = { loading: 'Carregando jogo', login_required: 'Login necessário', recovering: 'Recuperando sessão', online: 'Online', stale: 'Sem atividade recente', offline: 'Offline', error: 'Erro no painel', opened: 'Painel aberto', closed: 'Painel oculto' };
...
function darkGridBallIcon(alt = 'Pokébola') { const image = document.createElement('img'); image.className = 'darkgrid-ball-icon'; image.src = 'assets/darkgrid-pokeball.png'; image.alt = alt; image.draggable = false; return image; }
...
function describeOperationResult(response, fallback = 'operation_failed') { const result = response?.result || {}; if (result.operationStatus === 'partial') return `operação parcial: ${result.bought ?? result.soldCount ?? result.sold ?? 0} confirmado(s)`; if (result.operationStatus === 'completed' && result.returnStatus === 'failed') return 'operação concluída, mas retorno à hunt não confirmado'; return response?.reason || result.reason || fallback; }
async function buy1000Balls(account, button, ballId = 1) {
  ...
  try {
    ...
    if (!response?.ok || !response.result?.ok) throw new Error(describeOperationResult(response, 'purchase_failed'));
...
function render() {
  ...
  grid.replaceChildren(...state.accounts.map((account, index) => {
    ...
    const buy = document.createElement('button'); buy.className = 'mini-button buy-button'; buy.append(darkGridBallIcon(), document.createTextNode(' +1000')); buy.title = 'Comprar 1000 Pokébolas'; buy.disabled = Boolean(account.actionBusy); buy.addEventListener('click', () => buy1000Balls(account, buy));
    const balls = document.createElement('span'); balls.className = 'ball-count'; balls.append(darkGridBallIcon(), document.createTextNode(` ${Number(account.balls || 0).toLocaleString('pt-BR')}`)); balls.title = 'Pokébolas disponíveis';
...
function renderSimpleDashboard() {
  ...
  $('#simpleItems').replaceChildren(...accounts.map((account) => { const row = document.createElement('tr'); const name = document.createElement('td'); name.className = 'simple-account-cell'; name.textContent = `${account.name || account.label || 'Conta'} · ${Number(account.gold || 0).toLocaleString('pt-BR')}g`; row.append(name); supplies.forEach((supply) => { const found = findSupply(account, supply); const cell = document.createElement('td'); cell.className = 'simple-supply-cell'; const icon = supply.ball ? darkGridBallIcon(supply.label) : gameIcon(found.item?.icon, supply.label); if (icon) cell.append(icon); const count = document.createElement('span'); count.textContent = typeof found.quantity === 'string' ? found.quantity : Number(found.quantity || 0).toLocaleString('pt-BR'); cell.append(count); totals[supply.key] += typeof found.quantity === 'number' ? found.quantity : 0; if (supply.ball && found.item?.id) { const buy = document.createElement('button'); buy.className = 'simple-buy'; buy.append(darkGridBallIcon(), document.createTextNode('+1000')); buy.title = `Comprar 1000 ${supply.label}`; buy.addEventListener('click', (event) => { event.stopPropagation(); buy1000Balls(account, buy, found.item.id); }); cell.append(buy); } row.append(cell); }); row.addEventListener('click', () => openAccount(account)); return row; }));
...
async function submitOperation(event, action, inputFactory) {
  ...
  try {
    ...
    if (!response?.ok || !response.result?.ok) throw new Error(describeOperationResult(response));
