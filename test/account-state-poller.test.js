...
(async () => {
  ...
  const stalledPoller = new AccountStatePoller({ accountId: 'account-b', adapter: { async getState() { throw Object.assign(new Error('timeout'), { code: 'SURFACE_TIMEOUT' }); } }, maxFailures: 2, recoveryCooldownMs: 1000, shouldRecover: async () => true, recover: async () => { recoveries += 1; } });
  stalledPoller.on('error', () => {});
  stalledPoller.on('recovery', payload => recoveryStates.push(payload.status));
  ...
  assert.deepEqual(recoveryStates, ['recovering', 'online']);

  let loginRecovery = 0;
  const loginRequiredPoller = new AccountStatePoller({ accountId: 'account-login', adapter: { async getState() { return { status: 'login_required' }; } }, maxFailures: 1, shouldRecover: async () => true, recover: async () => { loginRecovery += 1; } });
  const loginStates = [];
  loginRequiredPoller.on('state', payload => loginStates.push(payload.state.status));
  await loginRequiredPoller.poll();
  await loginRequiredPoller.poll();
  loginRequiredPoller.dispose();
  assert.deepEqual(loginStates, ['login_required', 'login_required']);
  assert.equal(loginRecovery, 0, 'login_required não deve disparar reload/recovery automático');
