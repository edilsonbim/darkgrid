...
(async () => {
  const surface = new Surface({ execute: async (script) => {
    if (script.includes('collectorVersion')) return { ok: true, status: 'online', huntSlug: 'route-1', level: 42, gold: 500, balls: 20, inventory: [{ itemId: 1, name: 'Potion', quantity: 3 }], metrics: { kills: 4, xp: 100, captures: 2, shiny: 1, xph: 900, kph: 36, seconds: 400 } };
